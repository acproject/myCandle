...
use crate::dtype::{DType,IntDType,WithDType};
...
pub trait Map2 {
    ...
    fn map(
        ...
        match (v1, v2) {
            ...
            _ => std::result::Result::Err(Error::DTypeMismatchBinaryOp {
                lhs: v1.dtype(),
                rhs: v2.dtype(),
                op: Self::OP,
            }
...
pub fn unary_map<T: Copy, U: Copy, F: FnMut(T) -> U>(
    ...
    match layout.strided_blocks() {
        StridedBlocks::SingleBlock { start_offset, len } => vs
...
impl MatMul {
    fn striding_error(&self, lhs_l: &Layout, rhs_l: &Layout, msg: &'static str) -> Error {
        Error::MatMulUnexpectedStriding(Box::new(Error::MatMulUnexpectedStriding {
            lhs_l: lhs_l.clone(),
            rhs_l: rhs_l.clone(),
            bmnk: self.0,
            msg,
