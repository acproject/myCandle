...
impl<T: WithDType> TryFrom<&Tensor> for Vec<T> {
    type Error = Error;
    fn try_from(tensor: &Tensor) -> std::result::Result<Self, Self::Error> {
        tensor.to_vec1::<T>()
    }
}
...
macro_rules! from_tensor {
    ($typ:ident) => {
        ...
        impl TryFrom<$typ> for Tensor {
            ...
            fn try_from(v: $typ) -> std::result::Result<Self, Self::Error> {
...
impl Tensor {
    pub fn write_bytes<W: std::io::Write>(&self, f: &mut W) -> Result<()> {
