use std::ops::Deref;
use candle_core::{Tensor, Device, Result, DType, Var, Module};
use candle_nn::{Embedding, VarBuilder};
use cudarc::cublas::sys::cublasContext;
use libc::ssize_t;
use num_cpus::get;
...
impl BertEmbeddings {
    ...
    fn forward(&self, input_ids: &Tensor, token_type_ids:&Tensor) -> Result<Tensor> {
        ...
        let inpput_embeddings = self.word_embeddings.forward(input_ids)?;
        let token_type_embeddings = self.token_type_embeddings.forward(token_type_ids);
        let mut embeddings = (&inpput_embeddings + token_type_embeddings)?;
...
impl BertSelfAttention {
    fn load(vb:VarBuilder, config: &Config) -> Result<Self> {
        ...
        Ok(Self {
            ...
            attention_head_size,
            span:tracing::span!(tracing::Level::TRACE, "self-atten"),
